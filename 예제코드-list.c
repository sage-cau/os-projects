...
int listDir(char *arg)
{
   ...
   if((pdir = opendir(arg)) <= 0) {                      // 해당 디렉토리 열기
       perror("opendir");
       return -1;
   }
   ...
   while((dirt = readdir(pdir)) != NULL) {           // 현재 디렉토리를 읽을 수 있으면 순환
      ...
      sprintf(mtime, "%04d-%02d-%02d %02d:%02d:%02d",          // 출력을 위해 서식화
                      t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,    // tm 구조체는 뒤에서 설명
   ...
   for(i = 0; i < count; i++) {                                                // 다른 디렉토리에 대한 재귀호출
